...
Scanner.prototype.scan = function scan(cb) {
    ...
    const meteorVersions = path.resolve(process.cwd(), 'package-lock.json');
...
Scanner.prototype.walk = function walk(npmDependency, level, root) {
    ...
    if (npmDependency.name) {
        let package = root.packages[`node_modules/${npmDependency.name}`];
        if(!package){
        ...
        const dependency = new Dependency(
            ...
            npmDependency.licenses || npmDependency.license || package.license,
