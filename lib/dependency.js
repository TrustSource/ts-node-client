...
Dependency.prototype.addDependencies = function addDependencies(childs, ...args) {
    const toAdd = [];

    if (childs instanceof Dependency) { // called with (Dependency, Dependency, ...) as single argument
        toAdd.push(childs);
        args.forEach((d) => {
    ...
    } else if (Array.isArray(childs) && args.length === 0) { // called with ([Dependency])
        childs.forEach((d) => {
            if (d instanceof Dependency === false) {
                throw new TypeError('instance of Dependency as array element expected');
            }
            toAdd.push(d);
        });
    } else {
...
Dependency.getFirstByName = function getFirstByName(container, dependency) {
    ...
    } else if (container instanceof Object && container[dependency] instanceof Dependency) {
